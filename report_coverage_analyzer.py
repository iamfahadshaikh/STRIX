...
class ReportCoverageAnalyzer:
    ...
    def record_tool_executed(
        self, tool: str, endpoints: List[str], parameters: List[str], methods: List[str]
    ):
    ...
    def _create_coverage_gap(
        self, tool: str, category: str, reason: BlockReason
    ) -> CoverageGap:
    ...
    def get_coverage_report(self) -> Dict:
        ...
        return {
            ...
            "blocked": {
                ...
                "reasons": {
                    tool: reason.value for tool, reason in self.block_reasons.items()
                },
            },
            "coverage_gaps": [
                {
    ...
    def log_coverage_summary(self):
        ...
        logger.info(
            f"[Coverage] Executed: {summary['tools_executed']}, Blocked: {summary['tools_blocked']}"
        )
        logger.info(
            f"[Coverage] Tested: {summary['endpoints_tested']} endpoints, {summary['parameters_tested']} parameters"
        )

        if self.coverage.coverage_gaps:
            logger.warning(
                f"[Coverage] {len(self.coverage.coverage_gaps)} coverage gaps detected:"
            )
            for gap in self.coverage.coverage_gaps:
