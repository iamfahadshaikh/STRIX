...
class ResponseAnalysisResult:
    ...
    def risk_score(self) -> float:
        ...
        max_weight = max(
            weights.get(m.sensitivity, 0) for m in self.sensitive_matches
        )
        return max_weight
...
class ResponseAnalyzer:
    ...
    def analyze(self, status_code: int, body: str, 
                headers: Optional[Dict[str, str]] = None) -> ResponseAnalysisResult:
        """
        ...
        if result.is_json:
            try:
                ...
                result.analysis_notes.append("Content-Type claims JSON but body not valid JSON")
    ...
    def _analyze_json_fields(self, obj: Dict, result: ResponseAnalysisResult,
                            prefix: str = ""):
    ...
    def compare_responses(self, resp1: ResponseAnalysisResult, 
                         resp2: ResponseAnalysisResult) -> Dict:
        """
        ...
        return {
            ...
            "new_critical_data": len([m for m in new_sensitive if m.sensitivity == SensitivityLevel.CRITICAL]),
            "resp1_max_sensitivity": resp1.max_sensitivity.value if resp1.max_sensitivity else None,
            "resp2_max_sensitivity": resp2.max_sensitivity.value if resp2.max_sensitivity else None,
