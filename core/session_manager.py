...
class SessionManager:
    ...
    async def create_session(self, role: str, auth_type: AuthType,
        ...
        async with self._lock:
            session = SessionData(
                ...
                expires_at=datetime.now() + timedelta(
                    seconds=expires_in or self.default_session_timeout
                )
            )
    ...
    async def cleanup_expired(self):
        ...
        async with self._lock:
            expired_roles = [
                role for role, session in self._sessions.items()
