...
def test_classification():
    ...
    test_cases = [
        ...
        (
            "a.b.c.example.com",
            TargetType.MULTI_LEVEL_SUBDOMAIN,
            TargetScope.SINGLE_HOST,
        ),
    ]
...
def test_gating_logic():
    ...
    print(
        f"  Run subdomain enum: {context.should_run_subdomain_enum()} (expected: True)"
    )
    ...
    print(
        f"  Run subdomain enum: {context.should_run_subdomain_enum()} (expected: False)"
    )
...
def test_detection_gating():
    ...
    print(
        f"  Should run WordPress: {context.should_run_wordpress_tools()} (expected: False)"
    )
    ...
    print(
        f"  Should run WordPress: {context.should_run_wordpress_tools()} (expected: True)"
    )
