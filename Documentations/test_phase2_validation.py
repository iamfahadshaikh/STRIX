...
def test_cache_population():
    ...
    # Validate
    assert (
        len(cache.endpoints) == 2
    ), f"Expected 2 endpoints, got {len(cache.endpoints)}"
    assert len(cache.params) == 2, f"Expected 2 params, got {len(cache.params)}"
    assert (
        len(cache.reflections) > 0
    ), f"Expected reflections, got {len(cache.reflections)}"
...
def test_graph_building():
    ...
    assert (
        "/search" in reflectable
    ), f"Expected /search in reflectable, got {reflectable}"

    sql_injectable = graph.get_injectable_sql_endpoints()
    assert (
        "/api/users" in sql_injectable
    ), f"Expected /api/users in SQL injectable, got {sql_injectable}"

    parametric = graph.get_parametric_endpoints()
    assert (
        len(parametric) == 2
    ), f"Expected 2 parametric endpoints, got {len(parametric)}"
...
def test_crawler_mandatory_gate():
    ...
    ), f"Expected 1 endpoint, got {report['endpoints_discovered']}"
...
def test_parameter_flag_population():
    ...
    graph = EndpointGraph(target="https://example.com")
    graph.add_crawl_result(
        "/api/users", params={"search": ["test"], "cmd": ["ls"], "user_id": ["123"]}
    )
    graph.finalize()
    ...
    assert (
        graph.get_parameter("search").reflectable == True
    ), "Expected search to be reflectable"
    assert (
        graph.get_parameter("cmd").injectable_cmd == True
    ), "Expected cmd to be cmd-injectable"
    assert (
        graph.get_parameter("user_id").injectable_sql == True
    ), "Expected user_id to be SQL-injectable"
