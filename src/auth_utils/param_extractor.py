...
class ParameterExtractor:
    ...
    def _is_excluded_field(self, field_name: str) -> bool:
        ...
        return any(excl in field_lower for excl in self.EXCLUDE_FIELDS)

    def _classify_parameter(
        self, field_name: str, value: str
    ) -> Tuple[ParameterType, float]:
        """
        ...
        is_id_field = any(
            re.search(pattern, field_lower) for pattern in self.ID_FIELD_PATTERNS
        )
    ...
    def _infer_identifier_kind(self, field_name: str, value: str) -> str:
        ...
        if "email" in name:
            return "email"
        if any(
            token in name for token in ["id", "user", "account", "order", "resource"]
        ):
            return "identifier"
        return "generic"
    ...
    def get_id_parameters(self) -> List[Parameter]:
        ...
        return [
            p
            for p in self.extracted_params.values()
    ...
    def get_parameters_for_endpoint(self, endpoint: str) -> List[Parameter]:
        ...
        return [p for p in self.extracted_params.values() if p.endpoint == endpoint]
    ...
    def get_parameter_summary(self) -> Dict:
        ...
        return {
            ...
            "numeric_ids": len(
                [p for p in all_params if p.param_type == ParameterType.NUMERIC_ID]
            ),
