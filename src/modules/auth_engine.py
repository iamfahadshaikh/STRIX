...
class AuthEngine:
    ...
    def __init__(self, request_engine: RequestEngine, default_timeout: float = 30.0):
        """
        Args:
    ...
    async def _handle_form_login(
        self, role: str, config: LoginConfig, session: SessionData
    ) -> bool:
        """Handle form-based login"""
        try:
            response = await self.request_engine.post(
                config.login_url, data=config.login_data, timeout=self.default_timeout
            )
    ...
    async def _handle_bearer_token(
        self, role: str, config: LoginConfig, session: SessionData
    ) -> bool:
        """Handle Bearer token authentication"""
        try:
            token_info = TokenInfo(
                token=config.bearer_token, token_field="token", token_type="Bearer"
            )
    ...
    async def _extract_token_from_response(
        self, response: HTTPResponse, config: LoginConfig, session: SessionData
    ):
        ...
        token_value = self._get_nested_value(response_json, config.token_field_path)
        ...
        if token_value:
            ...
            logger.warning(
                f"Could not extract token using path: {config.token_field_path}"
            )

    async def _extract_cookies_from_response(
        self, response: HTTPResponse, config: LoginConfig, session: SessionData
    ):
            ...
            for cookie_name in cookies_to_extract:
                if cookie_name.lower() in cookie_def.lower():
                    ...
                    if "=" in cookie_def:
                        ...
                        cookies = {name: value}
                        await self.session_manager.update_cookies(session.role, cookies)
    ...
    async def get_authenticated_request(
        self, role: str, method: str, url: str, **kwargs
    ) -> HTTPResponse:
        """
