...
class PayloadCommandBuilder:
    ...
    def build_dalfox_commands(
        self, target_url: str, max_commands: int = 5
    ) -> List[Dict]:
        """
        ...
        if not self.graph:
            logger.warning(
                "[PayloadBuilder] No endpoint graph - cannot build dalfox commands"
            )
            return []
        ...
        for endpoint in reflectable_endpoints[:max_commands]:
            ...
            payloads = self.strategy.generate_xss_payloads(
                parameter=param.name, endpoint=endpoint, method=method
            )
            ...
            if payloads:
                ...
                logger.info(
                    f"[PayloadBuilder] Dalfox command generated: {endpoint}?{param.name}= ({len(payloads)} payloads)"
                )

        return commands

    def build_sqlmap_commands(
        self, target_url: str, max_commands: int = 3
    ) -> List[Dict]:
        """
        ...
        if not self.graph:
            logger.warning(
                "[PayloadBuilder] No endpoint graph - cannot build sqlmap commands"
            )
            return []
        ...
        if not injectable_endpoints:
            logger.warning(
                "[PayloadBuilder] No SQL-injectable endpoints - sqlmap skipped"
            )
            return []
        ...
        for endpoint in injectable_endpoints[:max_commands]:
            ...
            logger.info(
                f"[PayloadBuilder] SQLMap command generated: {endpoint}?{param.name}="
            )

        return commands

    def build_commix_commands(
        self, target_url: str, max_commands: int = 2
    ) -> List[Dict]:
        """
        ...
        if not self.graph:
            logger.warning(
                "[PayloadBuilder] No endpoint graph - cannot build commix commands"
            )
            return []
        ...
        for endpoint in parametric_endpoints[:max_commands]:
            ...
            logger.info(
                f"[PayloadBuilder] Commix command generated: {endpoint}?{param.name}="
            )
