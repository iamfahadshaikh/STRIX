...
class PayloadExecutionValidator:
    ...
    @staticmethod
    def validate_dalfox_execution(
        endpoint: str, parameter: str, method: str, crawler_data: Dict
    ) -> Tuple[bool, str]:
        """
    ...
    def validate_sqlmap_execution(
        ...
        if (
            parameter not in injectable_params
            and parameter not in dynamic_params
            and parameter not in all_params
        ):
    ...
    def validate_tool_execution(
        tool_name: str, endpoint: str, parameter: str, method: str, crawler_data: Dict
    ) -> Tuple[bool, str]:
    ...
    @staticmethod
    def validate_exploitation_proof(
        vuln_type: str, proof: Dict, confidence: float
    ) -> Tuple[bool, str]:
        ...
        if vuln in {"SQLI", "SQL", "SQL INJECTION"}:
            has_error = bool(
                proof.get("found_error") or proof.get("error_signature_found")
            )
            has_boolean = bool(
                proof.get("response_different") and proof.get("significant_diff")
            )
        ...
        if vuln == "XSS":
            ...
            exploitable = bool(
                proof.get("exploitable_context") or proof.get("vulnerable")
            )
        ...
        if vuln == "SSRF":
            ...
            has_metadata = any(
                k in indicator or k in evidence_text
                for k in ["metadata", "iam", "vpc", "subnet"]
            )
            has_file = any(
                k in indicator or k in evidence_text
                for k in ["file content", "root:", "windir", "system32"]
            )
...
class PayloadOutcomeTracker:
    ...
    def record_outcome(
        ...
        logger.info(
            f"[PayloadOutcome] {context.tool_name}: {outcome.value} on {context.endpoint}"
        )
    ...
    def get_summary(self) -> Dict:
        ...
        return {
            ...
            "confirmed_vulns": by_outcome.get(
                PayloadOutcome.EXECUTED_CONFIRMED.value, 0
            ),
