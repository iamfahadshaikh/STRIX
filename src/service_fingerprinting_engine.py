...
class ServiceFingerprintingEngine:
    ...
    def fingerprint_service(
        ...
        if service in ["HTTP", "HTTPS"] or protocol_type == "http":
            http_tech = self._fingerprint_http_service(
                host, port, protocol="https" if port == 443 else "http"
            )
            if http_tech:
    ...
    def _fingerprint_http_service(
        self, host: str, port: int, protocol: str = "http"
    ) -> Optional[Dict]:
        """
    ...
    def fingerprint_port_range(
        self, host: str, ports: List[int], use_common_only: bool = True
    ...
    def get_fingerprints_by_service(
