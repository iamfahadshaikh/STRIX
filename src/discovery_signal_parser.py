...
class DiscoverySignalParser:
            ...
            if matches:
                ...
                for port, proto, service in matches:
                    parsed_data["ports"].append(
                        {"port": int(port), "protocol": proto, "service": service}
                    )
    ...
    def parse_whatweb_output(stdout: str) -> Dict[str, any]:
        ...
        try:
            ...
            # Extract CMS
            cms_patterns = [r"WordPress", r"Drupal", r"Joomla", r"Magento", r"Shopify"]
...
def parse_and_extract_signals(tool_name: str, stdout: str, cache) -> bool:
    ...
    if not result["success"]:
        logger.warning(
            f"[{tool_name}] Signal extraction FAILED: {result.get('reason', 'unknown')}"
        )
        return False
    ...
    logger.info(
        f"[{tool_name}] Extracted {len(result['signals'])} signals: {result['signals']}"
    )
    return True
