...
class DeduplicationEngine:
    ...
    def deduplicate(self, findings: List[Dict]) -> List[Dict]:
        ...
        logger.info(
            f"[Deduplication] {len(findings)} → {len(deduplicated)} findings "
    ...
    def _get_dedup_key(self, finding: Dict) -> tuple:
        ...
        endpoint = self._normalize_endpoint(
            finding.get("endpoint") or finding.get("location", "")
        )
        parameter = self._normalize_parameter(finding.get("parameter", ""))
        category = self._normalize_vuln_type(
            finding.get("category") or finding.get("type", "")
        )
        method = (
            str(finding.get("method") or finding.get("http_method") or "GET")
            .strip()
            .upper()
        )
    ...
    def _normalize_vuln_type(self, vuln_type: str) -> str:
        ...
        normalized = (
            vuln_type.lower().replace("_", "").replace("-", "").replace(" ", "")
        )
    ...
    def _merge_duplicates(self, findings: List[Dict]) -> Dict:
        ...
        if "confidence" in primary:
            primary["confidence"] = min(
                100, primary["confidence"] + corroboration_boost
            )
        else:
        ...
        logger.debug(
            f"[Dedup] Merged {len(duplicates)} duplicates into 1 finding "
