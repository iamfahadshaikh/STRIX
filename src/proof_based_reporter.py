...
class ProofBasedReporter:
    ...
    def add_confirmed_vulnerability(
        ...
        remediation: str = "",
        impact: str = "",
    ) -> Optional[ConfirmedVulnerability]:
        """
        ...
        if confidence < self.MIN_CONFIDENCE_FOR_REPORTING:
            ...
            logger.warning(
                f"Rejected {vuln_type} finding: confidence {confidence:.2f} < {self.MIN_CONFIDENCE_FOR_REPORTING}"
            )
            return None

        proof = ProofOfExploitation(
            method=confirmation_method, confidence=confidence, details=details
        )
    ...
    def add_from_exploitation_result(
        self, exploitation_result: Union[Dict, Finding]
    ) -> Optional[ConfirmedVulnerability]:
        """
        ...
        confidence = max(
            0.0, min(1.0, confidence + strict_validation.confidence_adjustment)
        )
        ...
        severity = self._determine_severity(
            exploitation_result.get("type"), exploitation_result.get("endpoint")
        )
    ...
    def _determine_severity(self, vuln_type: str, endpoint: str) -> str:
        ...
        if any(
            x in endpoint.lower() for x in ["admin", "api", "panel", "user", "account"]
        ):
    ...
    def _generate_reproduction_steps(
        self, vuln_type: str, endpoint: str, parameter: str, payload: str
    ) -> List[str]:
    ...
    def _get_remediation(self, vuln_type: str) -> str:
        ...
        return remediations.get(
            vuln_type,
            "Review security controls and implement input validation/output encoding",
        )
    ...
    def _get_impact(self, vuln_type: str) -> str:
        ...
        return impacts.get(
            vuln_type, "Unauthorized access and potential system compromise"
        )
    ...
    def generate_report(self) -> Dict:
        ...
        """
        filtered_vulns = self._apply_false_positive_kill_switch(
        ...
        return {
            ...
            "statistics": {
                ...
                "critical_and_high": by_severity.get("CRITICAL", 0)
