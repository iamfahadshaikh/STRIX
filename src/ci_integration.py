...
class JUnitTestSuite:
    ...
    def to_xml(self) -> str:
        ...
        xml += (
            f'failures="{self.failures}" errors="{self.errors}" time="{self.time}">\n'
        )
...
class CIDDIntegration:
    ...
    def get_exit_code(self) -> ExitCode:
        ...
        if "error" in levels:
            return (
                ExitCode.CRITICAL_ISSUES
                if self.fail_on_critical
                else ExitCode.MEDIUM_ISSUES
            )
    ...
    def export_sarif(self, filepath: str) -> None:
        ...
        run = SARIFRun(tool_name=self.app_name, version=self.version)
        ...
        sarif = {"version": "2.1.0", "runs": [run.to_dict()]}
    ...
    def export_junit(self, filepath: str) -> None:
        ...
        level_to_status = {"error": "CRITICAL", "warning": "HIGH", "note": "LOW"}

        for issue in self.issues:
            status = level_to_status.get(issue.level, "INFO")
            endpoint = (
                issue.locations[0].get("endpoint", "unknown")
                if issue.locations
                else "unknown"
            )
            ...
            tc = JUnitTestCase(
                ...
                failure_text=(
                    json.dumps(issue.properties, indent=2) if issue.properties else ""
                ),
            )
    ...
    def print_summary(self) -> None:
        ...
        print(
            f"  Exit Code:        {summary['exit_code']} ({summary['exit_code_name']})"
        )
        print("=" * 70 + "\n")
...
class CIDDGateway:
    ...
    def evaluate(self, ci: CIDDIntegration) -> bool:
        ...
        if self.warn_on_medium and has_medium:
            logger.warning(
                "[CI/CD] Gate WARNING: Medium issues found (build continues)"
            )
