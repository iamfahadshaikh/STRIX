...
class StrictExecutionHandler:
    ...
    def should_proceed_to_exploitation(
        self, discovery_state: DiscoveryState, phase_name: str = "PHASE 5"
    ) -> Tuple[bool, str]:
        """
        ...
        if (
            discovery_state.endpoints_count == 0
            and discovery_state.controllable_params_count == 0
        ):
            ...
            checks.append(
                (
                    True,
                    f"Endpoints={discovery_state.endpoints_count}, Params={discovery_state.controllable_params_count}",
                )
            )
        ...
        if discovery_state.quality == DiscoveryQuality.FAILED:
            ...
            checks.append(
                (
                    True,
                    "Discovery quality FAILED but at least one source signaled; proceeding cautiously",
                )
            )
        elif discovery_state.quality == DiscoveryQuality.WEAK:
            reason = f"Discovery quality WEAK: {discovery_state.quality.value}"
            logger.warning(
                f"[{phase_name}] WARNING: {reason} (proceeding with caution)"
            )
        ...
        if not successful_sources:
            checks.append(
                (
                    True,
                    "No primary discovery source succeeded; continuing with low confidence",
                )
            )
        else:
...
def check_exploitation_readiness(
    discovery_state: DiscoveryState, phase_name: str = "PHASE 5"
) -> Tuple[bool, str]:
    """
