...
class SubdomainPrioritizationEngine:
    ...
    SUBDOMAIN_KEYWORDS = {
        SubdomainType.ADMIN: ["admin", "administrator", "manage", "dashboard", "control", "panel"],
    ...
    def assess_exposure_level(self, subdomain: str, 
                             subdomain_type: SubdomainType,
                             has_auth: bool = False,
                             tech_stack: Optional[List[str]] = None) -> ExposureLevel:
        """
        ...
        if subdomain_type in [SubdomainType.ADMIN, SubdomainType.API, SubdomainType.VPN]:
            if not has_auth:
    ...
    def calculate_priority_score(self, profile: SubdomainRiskProfile) -> float:
        ...
        if not profile.has_auth and profile.exposure_level in [ExposureLevel.CRITICAL, ExposureLevel.HIGH]:
            score *= 1.1
    ...
    def add_subdomain(self, subdomain: str,
                     ...
                     crawled_endpoints: int = 0) -> SubdomainRiskProfile:
        """
        ...
        # Assess exposure
        exposure = self.assess_exposure_level(subdomain, subdom_type, has_auth, tech_stack)
        ...
        logger.info(f"Added subdomain {subdomain}: type={subdom_type.value}, "
                   f"exposure={exposure.value}, score={profile.priority_score:.1f}")
        
        return profile
    
    def get_prioritized_list(self, limit: Optional[int] = None) -> List[SubdomainRiskProfile]:
        """
    ...
    def recommend_attack_order(self) -> List[Dict]:
        ...
        for profile in prioritized:
            ...
            recommendations.append({
                ...
                "reason": "; ".join(reason) if reason else "Potential attack surface"
            })
    ...
    def filter_by_type(self, subdom_type: SubdomainType) -> List[SubdomainRiskProfile]:
        ...
        return [p for p in self.subdomains.values() 
                if p.subdomain_type == subdom_type]
    ...
    def get_critical_targets(self) -> List[SubdomainRiskProfile]:
        ...
        for profile in self.subdomains.values():
            is_critical = (
                profile.exposure_level == ExposureLevel.CRITICAL or
                (profile.subdomain_type in [SubdomainType.ADMIN, SubdomainType.API] and 
                 not profile.has_auth) or
                profile.parameter_count > 10
            )
