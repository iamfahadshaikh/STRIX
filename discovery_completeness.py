...
class DiscoveryCompletenessEvaluator:
    ...
    def evaluate(self) -> CompletenessReport:
        ...
        if missing_critical:
            return CompletenessReport(
                ...
                completeness_score=len(present)
                / (len(self.CRITICAL_SIGNALS) + len(self.IMPORTANT_SIGNALS)),
    ...
    def _detect_present_signals(self) -> set:
        ...
        # DNS resolved?
        if (
            hasattr(self.cache, "discovered_ports")
            and len(self.cache.discovered_ports) > 0
        ):
    ...
    def _generate_recommendations(
        self, missing_critical: set, missing_important: set
    ) -> List[str]:
        ...
        if "reachable" in missing_critical:
            recommendations.append(
                "⏳ Target unreachable - check network connectivity or firewall rules"
            )

        if "ports_known" in missing_important:
            recommendations.append(
                "⏳ WAITING: nmap_quick running to discover open ports (required before payload tools)"
            )

        if "https" in missing_important and self.profile.is_web_target:
            recommendations.append(
                "⏳ Run sslscan or testssl to determine HTTPS availability"
            )
    ...
    def log_report(self, report: CompletenessReport):
        ...
        if report.complete:
            logger.info(
                f"[DiscoveryCompleteness] ✓ COMPLETE ({report.completeness_score:.0%})"
            )
            logger.info(f"[DiscoveryCompleteness] ✓ All payload tools ready to execute")
        else:
            logger.warning(
                f"[DiscoveryCompleteness] ⏳ INCOMPLETE ({report.completeness_score:.0%})"
            )
            logger.warning(
                f"[DiscoveryCompleteness] Waiting for: {', '.join(report.missing_signals)}"
            )
            logger.warning(
                f"[DiscoveryCompleteness] ⏳ Payload tools (dalfox, sqlmap, xsser, xsstrike, commix) BLOCKED until discovery completes"
            )
