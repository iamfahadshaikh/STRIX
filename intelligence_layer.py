...
class IntelligenceEngine:
    ...
    def calculate_confidence(
        self, finding: Finding, all_findings: List[Finding]
    ) -> ConfidenceScore:
        """
        ...
        for other in all_findings:
            ...
            if other.type == finding.type and self._locations_match(
                finding.location, other.location
            ):
    ...
    def _locations_match(self, loc1: str, loc2: str) -> bool:
        ...
        return (
            loc1_clean == loc2_clean
            or loc1_clean in loc2_clean
            or loc2_clean in loc1_clean
        )
    ...
    def filter_false_positives(self, findings: List) -> List:
        ...
        for finding in findings:
            ...
            tool = (
                finding.get("tool", "") if isinstance(finding, dict) else finding.tool
            )
    ...
    def _quantify_attack_surface(
        self, finding: Finding, related: List[Finding]
    ) -> float:
        """
        ...
        if not any(
            internal in finding.location.lower()
        ...
        return sorted(
            ...
            key=lambda cf: (
                ...
                -["CRITICAL", "HIGH", "MEDIUM", "LOW", "INFO"].index(
                    cf.primary_finding.severity.value
                ),
            ),
        )
    ...
    def generate_intelligence_report(self, correlated: List[CorrelatedFinding]) -> Dict:
        ...
        avg_confidence = (
            sum(cf.confidence.score for cf in correlated) / len(correlated)
            if correlated
            else 0
        )
        ...
        return {
            ...
            "high_confidence": len(
                [cf for cf in correlated if cf.confidence.score >= 0.8]
            ),
            "multi_tool_confirmed": len(
                [cf for cf in correlated if len(cf.confidence.confirming_tools) > 1]
            ),
            "by_exploitability": {
            ...
            "attack_surface": {
                ...
                "average_per_finding": (
                    round(total_surface / len(correlated), 2) if correlated else 0
                ),
            },
            "confidence_stats": {
                ...
                "high_confidence_ratio": (
                    round(
                        len([cf for cf in correlated if cf.confidence.score >= 0.8])
                        / len(correlated),
                        2,
                    )
                    if correlated
