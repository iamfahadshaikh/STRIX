...
def _attach_evidence_locations(
    findings: List[Finding], stdout: str, output_file: Optional[str]
) -> List[Finding]:
    ...
    def parse(stdout: str, target: str) -> List[Finding]:
        ...
        for match in re.finditer(port_pattern, stdout):
            ...
            if version and any(
                ...
                for old in ["outdated", "deprecated", "unsupported"]
            ):
                findings.append(
                    Finding(
...
class GobusterParser:
    ...
    def parse(stdout: str, target: str, tool_name: str = "gobuster") -> List[Finding]:
        ...
        sensitive_paths = {
            ...
            "phpinfo": (
                Severity.HIGH,
                "phpinfo() page exposed (information disclosure)",
            ),
...
class TestSSLParser:
    ...
    def parse(stdout: str, target: str) -> List[Finding]:
        ...
        for line in stdout.splitlines():
            ...
            if "breach" in lower_line and (
                "potentially" in lower_line or "gzip" in lower_line
            ):
                findings.append(
                    Finding(
                        type=FindingType.WEAK_CRYPTO,
...
class WPScanParser:
    ...
    def parse(stdout: str, target: str) -> List[Finding]:
        ...
        version_pattern = (
            r"WordPress version\s+([\d.]+)\s+identified.*?The version is out of date"
        )
...
def parse_tool_output(
    tool: str, stdout: str, stderr: str, target: str, output_file: Optional[str] = None
) -> List[Finding]:
    """
