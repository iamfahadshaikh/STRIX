...
from core.session_manager import SessionManager, AuthType
...
from findings_model import FindingsRegistry, Finding, Severity
...
class AuthenticationAndIDOROrchestrator:
    ...
    async def load_config(self, config_path: str) -> bool:
        ...
        try:
            ...
            logger.info(f"Loaded {len(config.get('authentication_engines', []))} auth configs")
            return True
    ...
    async def test_access_control(self, endpoints: List[str], 
                                 roles: List[str]) -> int:
        ...
        for endpoint in endpoints:
            ...
            findings = await self.access_control_engine.test_privilege_escalation(
                endpoint,
                low_role=roles[0] if roles else "user",
                high_role="admin"
            )
            ...
            unauth_findings = await self.access_control_engine.test_unauthorized_endpoint_access(
                endpoint
            )
            if unauth_findings:
        ...
        try:
            ...
            report = {
                ...
                "idor_findings": [
                    ...
                    for f in all_findings if f.type.value == "IDOR"
                ],
                "access_control_findings": [
                    ...
                    for f in all_findings if f.type.value == "Authentication Bypass"
                ],
                "statistics": {
    ...
    async def run_full_assessment(self, config_path: str,
                                 endpoints: List[str],
                                 roles: List[str],
