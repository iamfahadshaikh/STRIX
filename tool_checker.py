...
class InteractiveToolChecker:
    ...
    def check_and_prompt(self):
        ...
        print(f"\n{'='*80}")
        print(
            f"SUMMARY: {installed_count}/{total_count} tools installed, {missing_count} missing"
        )
        print(f"{'='*80}\n")
    ...
    def prompt_for_installation(self):
        ...
        print(
            f"Found {sum(len(v) for v in missing_by_category.values())} missing tools.\n"
        )
        ...
        for category in sorted(missing_by_category.keys()):
            ...
            for tool, info in sorted(missing_by_category[category]):
                ...
                while True:
                    response = (
                        input(f"   Install {tool}? [y/n/a(ll)/s(kip all)]: ")
                        .strip()
                        .lower()
                    )
                    if response in ["y", "n", "a", "s"]:
                        if response == "y":
                            ...
                            for remaining_tool, remaining_info in sorted(
                                missing_by_category[category]
                            ):
                                if (
                                    remaining_tool != tool
                                    and self.tool_manager.get_install_command(
                                        remaining_tool
                                    )
                                ):
                            ...
                            current_category_index = sorted(
                                missing_by_category.keys()
                            ).index(category)
                            for next_category in sorted(missing_by_category.keys())[
                                current_category_index + 1 :
                            ]:
                                for next_tool, next_info in sorted(
                                    missing_by_category[next_category]
                                ):
        ...
        for i, tool in enumerate(self.selected_tools, 1):
            ...
            try:
                ...
                result = subprocess.run(
                    command, shell=True, capture_output=False, text=True, timeout=600
                )
