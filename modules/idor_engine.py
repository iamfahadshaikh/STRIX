...
from core.request_engine import RequestEngine, HTTPResponse, RequestMethod
from auth_utils.param_extractor import Parameter, ParameterType, ParameterExtractor
from auth_utils.response_analyzer import ResponseAnalyzer, SensitivityLevel, ResponseAnalysisResult
from findings_model import Finding, FindingType, Severity, FindingsRegistry
...
class IDOREngine:
    ...
    async def test_endpoint(self, endpoint: str, method: str = "GET",
                           ...
                           parameter_names: Optional[List[str]] = None) -> List[IDORFinding]:
        """
        ...
        params_to_test = parameter_names or await self._extract_params_from_endpoint(endpoint)
                ...
                for mutation in mutations:
                    try:
                        ...
                        idor_finding = await self._analyze_for_idor(
                            endpoint, param, mutation,
                            baseline_resp, mutated_resp, role
                        )
    ...
    async def _get_baseline_response(self, endpoint: str, method: str,
                                    param: Parameter, role: str) -> Optional[HTTPResponse]:
    ...
    def _generate_mutations(self, param: Parameter, endpoint: str) -> List[MutationStrategy]:
        ...
        if param.param_type == ParameterType.NUMERIC_ID:
            ...
            mutations = [
                MutationStrategy.RANDOM_ID,
                MutationStrategy.KNOWN_IDS if param.name in self.seen_ids else MutationStrategy.RANDOM_ID
            ]
    ...
    async def _make_mutated_request(self, endpoint: str, method: str,
                                   param: Parameter, mutation_strategy: MutationStrategy,
                                   role: str) -> Optional[HTTPResponse]:
        """Make request with mutated parameter"""
        try:
            ...
            mutated_endpoint = endpoint.replace(
                f"{param.name}={param.example_value}",
                f"{param.name}={mutated_value}"
            )
    ...
    async def _analyze_for_idor(self, endpoint: str, param: Parameter,
                               mutation_strategy: MutationStrategy,
                               baseline_resp: HTTPResponse,
                               mutated_resp: HTTPResponse,
                               role: str) -> Optional[IDORFinding]:
        """
        ...
        analysis = self.response_analyzer.analyze(
            mutated_resp.status_code,
            mutated_resp.body,
            mutated_resp.headers
        )
        ...
        if comparison["new_critical_data"] == 0 and analysis.max_sensitivity != SensitivityLevel.HIGH:
        ...
        finding = IDORFinding(
            ...
            affected_data_sensitivity=analysis.max_sensitivity.value if analysis.max_sensitivity else None,
            is_confirmed=True
        )
        
        logger.warning(
            f"IDOR FOUND: {endpoint} - {param.name} parameter - Role {role}"
        )
    ...
    def get_statistics(self) -> Dict:
        ...
        return {
            ...
            "confirmed_idor": len([f for f in self.findings_discovered if f.is_confirmed]),
            "by_severity": {
                "CRITICAL": len([f for f in self.findings_discovered if f.severity == Severity.CRITICAL]),
                "HIGH": len([f for f in self.findings_discovered if f.severity == Severity.HIGH]),
                "MEDIUM": len([f for f in self.findings_discovered if f.severity == Severity.MEDIUM]),
            }
        }
