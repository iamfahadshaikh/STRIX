...
class AccessControlViolationType(Enum):
    ...
    UNAUTHORIZED_ENDPOINT = "unauthorized_endpoint"      # Not authenticated user accessing admin endpoint
...
class AccessControlFinding:
    ...
    def to_finding(self) -> Finding:
        ...
        return Finding(
            ...
            cwe="CWE-434" if self.violation_type == AccessControlViolationType.UNAUTHORIZED_ENDPOINT else "CWE-639",
            owasp="A01:2021",
...
class AccessControlEngine:
    ...
    async def test_privilege_escalation(self, endpoint: str,
                                       low_role: str = "user",
                                       high_role: str = "admin") -> List[AccessControlFinding]:
        """
        ...
        """
        findings = []
        logger.info(f"Testing privilege escalation: {low_role} → {high_role} on {endpoint}")
    ...
    async def test_unauthorized_endpoint_access(self, endpoint: str,
                                               required_role: str = "admin") -> List[AccessControlFinding]:
        """
    ...
    async def test_method_tampering(self, endpoint: str,
                                   role: str = "user") -> List[AccessControlFinding]:
        """
        ...
        successful_methods = [m for m, status in results.items() if status and 200 <= status < 300]
        failed_methods = [m for m, status in results.items() if status and status >= 400]
    ...
    async def test_role_header_bypass(self, endpoint: str,
                                     low_role: str = "user",
                                     high_role: str = "admin") -> List[AccessControlFinding]:
        """
    ...
    async def test_common_admin_endpoints(self, roles: List[str]) -> List[AccessControlFinding]:
        """
