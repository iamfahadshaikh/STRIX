...
class Parameter:
    ...
    def __repr__(self):
        return (
            f"Parameter(name={self.name}, type={self.param_type.value}, "
...
class ParameterIntelligenceEngine:
    ...
    def classify_parameters(
        self, endpoint: str, method: str, params: List[Dict]
    ) -> List[Parameter]:
        """
    ...
    def _classify_single(
        self, param_name: str, endpoint: str, method: str, source: str, value: str = ""
    ) -> Parameter:
    ...
    def filter_controllable(self, parameters: List[Parameter]) -> List[Parameter]:
        ...
        logger.info(
            f"Filtered {len(rejected)} metadata params, {len(controllable)} controllable remain"
        )
        for p in rejected:
    ...
    def filter_high_confidence(
        self, parameters: List[Parameter], threshold: float = 0.70
    ) -> List[Parameter]:
        ...
        logger.info(
            f"Filtered by confidence ({threshold}): {len(high_conf)} remain, {len(low_conf)} rejected"
        )
        for p in low_conf:
    ...
    def get_priority_params(
        self, parameters: List[Parameter], top_n: int = 5
    ) -> List[Parameter]:
    ...
    def summary(self, parameters: List[Parameter]) -> Dict:
        ...
        return {
            ...
            "avg_exploitability": (
                sum(p.exploitability_score for p in controllable) / len(controllable)
                if controllable
                else 0
            ),
        }
...
def classify_parameters(
    endpoint: str, method: str, params: List[Dict]
) -> List[Parameter]:
...
def filter_high_confidence(
    parameters: List[Parameter], threshold: float = 0.70
) -> List[Parameter]:
