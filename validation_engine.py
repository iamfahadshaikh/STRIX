...
class ValidationResult:
    ...
    def __init__(self, valid: bool, reason: str, confidence_adjustment: float = 0.0):
        ...
        self.confidence_adjustment = (
            confidence_adjustment  # Boost/penalty to confidence
        )
...
class ValidationEngine:
    ...
    def validate(
        self, vuln_type: str, proof: Dict, confidence: float
    ) -> ValidationResult:
        """
        ...
        if vuln_enum == VulnerabilityType.SSRF:
            ...
            if confidence >= 0.90:
                return ValidationResult(
                    True,
                    f"Unknown type {vuln_type} passes high-confidence threshold",
                    0.0,
                )
            else:
                return ValidationResult(
                    False,
                    f"Unknown type {vuln_type}, confidence {confidence} < 0.90",
                    -0.1,
                )
        ...
        if response_body:
            for pattern in self.internal_ip_patterns:
                if re.search(pattern, response_body):
                    return ValidationResult(
                        True, f"SSRF confirmed: Internal IP detected in response", +0.15
                    )
    ...
    def _validate_xss(self, proof: Dict, confidence: float) -> ValidationResult:
        ...
        reflection_context = proof.get(
            "reflection_context", ""
        )  # "html" | "js" | "attr" | "comment"
    ...
    def _validate_command_injection(
        self, proof: Dict, confidence: float
    ) -> ValidationResult:
        """
        ...
        if error_output and any(
            err in error_output
            for err in ["command not found", "SyntaxError", "unexpected"]
        ):
            return ValidationResult(
                True,
