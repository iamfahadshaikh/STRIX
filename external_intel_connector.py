...
class CrtShConnector:
    ...
    def query_domain(domain: str, timeout: int = 10) -> ExternalIntelResult:
        ...
        try:
            ...
            logger.info(
                f"[crt.sh] Found {len(results)} certificate entries for {domain}"
            )
...
class ShodanConnector:
    ...
    def query_host(self, ip: str, timeout: int = 10) -> ExternalIntelResult:
        ...
        try:
            ...
            return ExternalIntelResult(
                source="shodan", data_type="host_info", results=results, success=True
            )
    ...
    def query_host(self, ip: str, timeout: int = 10) -> ExternalIntelResult:
        ...
        try:
            ...
            response = requests.get(
                url, auth=(self.api_id, self.api_secret), timeout=timeout
            )
            response.raise_for_status()
            ...
            return ExternalIntelResult(
                source="censys", data_type="host_info", results=results, success=True
            )
...
class ExternalIntelAggregator:
    ...
    def __init__(
        ...
        self.censys = (
            CensysConnector(api_id=censys_id, api_secret=censys_secret)
    ...
    def to_cache_signals(
        self, intel_results: Dict[str, ExternalIntelResult], cache
    ) -> None:
        ...
        logger.info(
            f"[ExternalIntel] Added signals from {len([r for r in intel_results.values() if r.success])} sources"
        )
