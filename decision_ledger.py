...
class DecisionLedger:
    ...
    def allows(self, tool_name: str) -> bool:
        ...
        if tool_name not in self.decisions:
            raise KeyError(
                f"Tool {tool_name} not in decision ledger (architecture violation)"
            )

        return self.decisions[tool_name].decision in (
            Decision.ALLOW,
        ...
        return {
            ...
            "decisions": {
                name: {
                    ...
                    "prerequisites": (
                        sorted(decision.prerequisites)
                        if isinstance(decision.prerequisites, set)
                        else decision.prerequisites
                    ),
    ...
    def record_tool_decision(
        self, tool_name: str, decision: Decision, reason: str
    ) -> None:
    ...
    def get_crawl_gating_summary(self, crawl_adapter) -> dict:
        ...
        for tool in ["xsstrike", "dalfox", "sqlmap", "commix"]:
            ...
            allowed_by_crawl = self.should_run_payload_tool_with_crawl(
                tool, crawl_adapter
            )
            ...
            if can_run:
                if tool in ["xsstrike", "dalfox"]:
                    reason = (
                        f"XSS testing ({gating['reflection_count']} reflection targets)"
                    )
                elif tool == "sqlmap":
                    reason = f"SQL injection ({gating['parameter_count']} parameters)"
                elif tool == "commix":
                    reason = (
                        f"Command injection ({gating['parameter_count']} parameters)"
                    )
                else:
...
class DecisionEngine:
    ...
    def build_ledger(profile) -> DecisionLedger:
        ...
        if profile.is_ip:
            ...
            ledger.add_decision(
                "dnsrecon", Decision.DENY, "Target is IP (no DNS needed)"
            )
        elif profile.is_subdomain:
            # Subdomains inherit all DNS records from root domain (via dnsrecon)
            ledger.add_decision(
                "dnsrecon", Decision.DENY, "Full DNS recon not needed for subdomain"
            )
        else:
            # Root domains get full DNS via dnsrecon
            ledger.add_decision(
                "dnsrecon", Decision.ALLOW, "Full DNS recon for root domain", priority=8
            )
        ...
        if profile.is_root_domain:
            ...
            ledger.add_decision(
                "findomain",
                Decision.ALLOW,
                "Subdomain enumeration (root domain)",
                priority=15,
            )
            ledger.add_decision(
                "sublist3r",
            ...
            ledger.add_decision(
                "findomain", Decision.DENY, "Not applicable for non-root domain"
            )
            ledger.add_decision(
                "sublist3r", Decision.DENY, "Not applicable for non-root domain"
            )
            ledger.add_decision(
                "assetfinder", Decision.DENY, "Not applicable for non-root domain"
            )

        # ============ NETWORK SCANNING ============
        ledger.add_decision("ping", Decision.ALLOW, "Reachability check", priority=5)
        ledger.add_decision(
            "nmap_quick", Decision.ALLOW, "Quick port scan", priority=20, timeout=9999
        )
        ledger.add_decision(
            "nmap_vuln",
            Decision.ALLOW,
            "Vulnerability-focused nmap",
            priority=19,
            timeout=9999,
        )
        ...
        # ============ WEB DETECTION ============
        ledger.add_decision(
            "whatweb", Decision.ALLOW, "Early web technology detection", priority=25
        )
        ledger.add_decision(
            "whatweb_http_fallback",
            Decision.ALLOW,
            "HTTP fallback for web tech detection",
            priority=25,
        )
        ...
        if profile.is_https and profile.is_web_target:
            ledger.add_decision(
                "sslscan", Decision.ALLOW, "SSL/TLS certificate analysis", priority=18
            )
            ledger.add_decision(
                "testssl", Decision.ALLOW, "SSL/TLS vulnerability scan", priority=17
            )
            ledger.add_decision(
                "openssl_connect",
                Decision.ALLOW,
            ...
            ledger.add_decision(
                "openssl_status", Decision.ALLOW, "OpenSSL OCSP status", priority=14
            )
            ledger.add_decision(
                "openssl_state", Decision.ALLOW, "OpenSSL handshake state", priority=13
            )
        else:
        ...
        ledger.add_decision(
            "gobuster",
            Decision.ALLOW,
            "Directory enumeration (all targets)",
            priority=16,
            timeout=9999,
        )
        ledger.add_decision(
        ...
        ledger.add_decision(
            "dirsearch", Decision.ALLOW, "Directory/file search", priority=15
        )
        ledger.add_decision(
            "nikto",
            Decision.ALLOW,
            "Baseline web scan (all targets)",
            priority=14,
            timeout=400,
        )
        ...
        if profile.has_wordpress:
            ledger.add_decision(
                "wpscan", Decision.ALLOW, "WordPress vulnerability scanner", priority=12
            )
        else:
        ...
        ledger.add_decision(
            "commix",
            Decision.ALLOW,
            "Command injection testing (all targets)",
            priority=8,
            timeout=9999,
        )
        ...
        if profile.has_reflection:
            ledger.add_decision(
                "xsser", Decision.ALLOW, "XSS testing (reflection detected)", priority=7
            )
        else:
        ...
        ledger.add_decision(
            "nuclei_crit",
            Decision.ALLOW,
            "Nuclei critical templates (all targets)",
            priority=6,
            timeout=9999,
        )
        ledger.add_decision(
            "nuclei_high",
            Decision.ALLOW,
            "Nuclei high severity templates (all targets)",
            priority=5,
            timeout=9999,
        )
        ledger.add_decision(
            "nuclei_all",
            Decision.ALLOW,
            "Nuclei all templates (all targets)",
            priority=5,
            timeout=9999,
        )
        ledger.add_decision(
            "nuclei_cves",
            Decision.ALLOW,
            "Nuclei CVE templates (all targets)",
            priority=6,
            timeout=9999,
        )
        ledger.add_decision(
            "nuclei_ssl",
            Decision.ALLOW,
            "Nuclei SSL templates (all targets)",
            priority=5,
            timeout=9999,
        )
